...
const spriteByDexNumber = new Map(
  Object.values(emeraldTeamData.species).map((species) => [species.dexNumber, species.sprite])
);
...
function addEncounterSprites(methods) {
  return methods.map((method) => ({
    ...method,
    pokemon: method.pokemon.map((pokemon) => ({
      ...pokemon,
      sprite: spriteByDexNumber.get(pokemon.dexNumber)
    }))
  }));
}

function addCaptureStatus(methods, ownedFlags) {
  return addEncounterSprites(methods).map((method) => ({
    ...method,
    pokemon: method.pokemon.map((pokemon) => ({
      ...pokemon,
